...
class GUI(UI):
    def __init__(
        ...
        self.width, self.height = \
            self.life.cell_width * self.life.cell_size, self.life.cell_height * self.life.cell_size
    ...
    def draw_lines(self) -> None:
        """ Отрисовать сетку """
        for x in range(0, self.width, self.life.cell_size):
            pygame.draw.line(self.screen, pygame.Color(
                "black"), (x, 0), (x, self.height))
        for y in range(0, self.height, self.life.cell_size):
            pygame.draw.line(self.screen, pygame.Color(
        ...
        for row_index in range(self.life.cell_height):
            for col_index in range(self.life.cell_width):
                ...
                pygame.draw.rect(
                    surface=self.screen,
                    color=color,
                    rect=(row_index * self.life.cell_size, col_index * self.life.cell_size,
                          self.life.cell_size, self.life.cell_size)
                )
